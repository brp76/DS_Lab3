...
public final class ArrayBag<T> implements BagInterface<T> {
    ...
    // MODIFY THIS METHOD TO REMOVE A RANDOM ITEM FROM THE BAG
        ...
        if (numberOfEntries <= 0) {
        	...
        	int givenIndex = 0;
        	result = removeEntry(givenIndex);
        } else {
        	int givenIndex = rng.nextInt(numberOfEntries-1);
            if (!isEmpty() && givenIndex >= 0){
            	result = removeEntry(givenIndex);	// Remove at random index
            }
        }
        return result;
    } // end remove
...
// equals bag[where]; otherwise, anEntry is not in the array
        	...
        	for (int i = 0; i < numberOfEntries; i++) {
        		for (int j = 0; j < numberOfEntries; j++) {
        			if (bag[i] == aBag.bag[j]) {
        				...
        				int freq2 = getFrequencyOf(aBag.bag[j]);
        				if (freq1 == freq2) {
        					result = true;
        				} else {
        					result = false;
    ...
    public boolean duplicateAll() {
        ...
        int tempSize = numberOfEntries;
        if (numberOfEntries*2 > bag.length) {
        	System.out.println("Attempt to create a bag whose capacity exceeds allowed maximum.");
        	success = false;
        } else {
    ...
    public void removeDuplicates() {
        ...
        // COMPLETE THIS METHOD 
        for (int i = 0; i < numberOfEntries; i ++) { // Outer loop
        	for (int j = 0; j < numberOfEntries; j ++) {
        		if (i == j) {
        			// Do Nothing
        		} else {
        			while (bag[i] == bag[j]) {
        				bag[j] = bag[numberOfEntries-1];
